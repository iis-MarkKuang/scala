...
case class Book(
                ...
                solicited_periodical: String = "",
                batch: String = "",
                serial_number: Int = 0,
                last_check_time: String = Time.now.toString,
                description: String = "",
...
object BookWithId {
  ...
  def toDomain(id: String, book:Book ): BookWithId = {
    BookWithId(id, book.barcode, book.rfid, book.reference, book.category, book.title, book.stack, book.clc,
      ...
      book.is_active, book.user, book.shelf, book.act_shelf, book.last_check_id, book.solicited_periodical, book.batch, book.serial_number, book.description, book.datetime)
  }
}
...
object CheckItem {
  implicit val checkItemFmt = Json.format[CheckItem]
}

case class CheckItem(barcode: String,
                     shelf: String,
                     act_shelf: String,
                     timestamp: String
                    )
...
object CheckObject {
  implicit val checkFmt = Json.format[CheckObject]
}

case class CheckObject(check_id: String,
                 data: Seq[CheckItem]
                )