...
import com.shrfid.api.http.Elastic.data.GetReaderBorrowRankingRequest
...
class ReaderService @Inject()(authService: AuthService,
  ...
  def findMemberByBarcode(request: GetReaderMemberByBarcodeRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.findReaderMemberByBarcode)
  }
  ...
  def reservationItems(request: PostReaderMemberReservationItemsRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.reservationItems)
  }
  ...
  def findReaderBorrowRanking(request: GetReaderBorrowRankingRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.findReaderBorrowRanking)
  }

  def getReaderDelayedFine(request: GetReaderMemberDelayedFineRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.getReaderDelayedFine)
  }

  def getReaderLostFine(request: GetReaderMemberLostFineRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.getReaderLostFine)
  }

  def updateReaderCredit(request: UpdateReaderMemberCreditByIdRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.updateReaderCredit)
  }

  // Added by kuang 4/18/2017
  def insertNewLevel(request: PostReaderLevelNewRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.insertNewReaderLevel)
  }

  def findNewLevels(request: GetReaderLevelListRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.findNewReaderLevels)
  }

  def findNewLevelById(request: GetReaderLevelByIdRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.findNewReaderLevelById)
  }

  def updateNewLevelById(request: PatchReaderLevelNewByIdRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.updateNewReaderLevelById)
  }

  def deleteNewLevelById(request: DeleteReaderLevelByIdRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.deleteNewReaderLevelById)
  }

  def deleteNewLevelBulk(request: DeleteReaderLevelBulkRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.deleteNewReaderLevelBulk)
  }

  // Added by kuangyuan 4/19/2017
  def borrowItemsNew(request: PostReaderMemberBorrowItemsRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.borrowItemsNew)
  }

  def renewItemsNew(request: PostReaderMemberRenewItemsRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.renewItemsNew)
  }

  def reserveItemsNew(request: PostReaderMemberReservationItemsRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.reservationItemsNew)
  }

  // Added by kuangyuan 5/9/2017
  def getReaderMemberDelayedDetail(request: GetReaderMemberDelayedDetailRequest, permission: PermissionCode) = {
    tokenService.checkPermissionWithCallBack(token(request.Authorization), permission)(request, elasticService.getReaderMemberDelayedDetail)
  }
}
