...
class MysqlServiceImpl @Inject()(authUserRepository: AuthUserRepository,
  ...
  override def deleteAuthGroupNew(id: StatusCode): Future[(Int, String)] = {
    authUserGroupRepository.dal.findUserIdsInAuthGroup(id) flatMap {
      case empty if empty.length == 0 =>
        for {
          r <- authGroupRepository.dal.deleteById(id)
        } yield (r, DeleteSucceed)
      case _ => Future.value((0, AuthGroupNotEmpty))
    }
  }
